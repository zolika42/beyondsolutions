...
class LoggerClass
{
     ...
     */
    private function logToOutput(string $level, string $message): string
    {
        if ($this->environment !== 'dev') {
            return '';
        }
     ...
     */
    public function getLastLines(int $lines = 10): array
    {
        if (!file_exists($this->logFile)) {
        ...
        return array_reverse($logMessages); // Reverse to show newest first
    }
}
